...
<template>
  <div class="page-block-pos-list">
    <div :class="'pos-list-item' + (item.boxShadow === false ? ' no-shadow' : '')" v-for="(item, idx) in (datasetStore)" :key="idx" :data-id="item.id">
      <div class="list-item-mask j-item-mask" :data-id="item.id" v-if="item.mask"></div>
      ...
      <div :class="'pos-item-fd' + (item.body.type === 'list' ? ' fd-item-list' : '') + (!item.head && item.split === false ? ' border-radius' : '')"
        ...
        <div class="fd-item" v-if="item.body.type === 'input'"
          ...
          <label :class="'fd-item-lab ' + (group.labelType)">{{group.label}}</label>
          <span class="fd-item-inp">
            <input :type="group.modelType || 'text'" v-model="group.vModel" :placeholder="group.modelPlaceHolder" @blur="inputBlurEvent" />
          </span>
        ...
        <div :class="'fd-item ' + group.itemClass" v-if="item.body.type === 'list'"
          v-for="(group, gidx) in item.body.groups" :key="gidx">
          <label :class="'fd-item-lab ' + (group.labelType)">{{group.label}}</label>
          <label class="fd-item-txt">{{group.text}}</label>
        </div>
        <div :class="'fd-item ' + group.itemClass" v-if="item.body.type === 'txt-input'"
          v-for="(group, gidx) in item.body.groups" :key="gidx">
          <div v-if="group.modelType === 'txt'" >
            <label :class="'fd-item-lab ' + (group.labelType)">{{group.label}}</label>
            <label class="fd-item-txt">{{group.text}}</label>
          </div>
          <div v-else>
            <label :class="'fd-item-lab ' + (group.labelType)">{{group.label}}</label>
            <span class="fd-item-inp">
              <input :type="group.modelType || 'text'" v-model="group.vModel" :data-verify="group.verify" :data-idx="gidx" :placeholder="group.modelPlaceHolder" @blur="inputBlurEvent" @keyup="inputKeyupEvent" @keydown="inputKeydownEvent" @pause="inputPauseEvent" />
            </span>
            <label class="fd-item-remark">{{group.remark}}</label>
          </div>
        </div>
