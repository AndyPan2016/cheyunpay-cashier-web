...
let render = {
  ...
  methods: {
    ...
    tip (msg, timeout, callBack) {
      ...
      this.popupContent = msg || '操作失败，请稍后再试'
      ...
      if (utils.hasClass(target, 'j-popup-btn')) {
        ...
        if (btnType) {
          ...
          if (utils.isFunction(buttonFn)) {
            if (utils.isFalse(buttonFn.call(target, e, resultData, promptModel))) {
              return false
            }
          }
              ...
              let eachPickerTarget = (className, targetNode) => {
                ...
                let each = (target) => {
                  ...
                  for (let a = 0, len = child.length; a < len; a++) {
                    ...
                    if (utils.hasClass(childItem, clas)) {
            ...
            if (this.pickerType) {
              ...
              setTimeout(() => {
                ...
                if (utils.isFunction(onSure)) {
                  if (utils.isFalse(onSure.call(target, e, resultData, that.pickerDateModel, pickerDatas))) {
                    return false
                  }
                }
              }, 100)
              ...
              if (utils.isFunction(onSure)) {
                if (utils.isFalse(onSure.call(target, e, resultData, pickerDatas))) {
                  return false
                }
              }
            }
    ...
    pickerChange (picker, value, index) {
      ...
      if (this.cityLinkage) {
        ...
        if (index === 0) {
          ...
          let temp2 = []
          utils.forEach(city[0].county, (item) => {
            temp2.push(item)
          })
    ...
    renderCityMap () {
      if (this.cityLinkage) {
        let cityMapData = Utils.require(Utils.city).default
        this.cityMapData = cityMapData
        let tempProvince = {}
        // 省、市、区(县)
        let mockCityData = [{values: []}, {values: []}, {values: []}]
        utils.forEach(cityMapData, (cityItem) => {
        ...
        let tempCity = {}
        utils.forEach(cityMapData[0].city, item => {
          mockCityData[1].values.push(item.name)
          tempCity[item.name] = item
        })
        this.tempCity = tempCity

        utils.forEach(cityMapData[0].city[0].county, item => {
